<template>
  <div class="admin-wrapper">
    <div class="content">
            ...
            <tbody>
              <tr v-for="imp in impianti" :key="imp.pod_id">
                ...
                <td class="text-right">
                  ...
                  <button @click="apriModaleGrafico(imp.pod_id)" class="btn-text text-green">
                    <span class="flex-align" style="justify-content: flex-end; gap: 4px;">
                  ...
                  <button @click="apriModaleModifica(imp)" class="btn-text text-blue ml-3">Modifica</button>
                  <button @click="eliminaImpianto(imp.pod_id)" class="btn-text text-red ml-3">Elimina</button>
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <div v-if="modaleGraficoAperto" class="modal-overlay">
        <div class="admin-card modal-content modal-lg">
          <div class="modal-header">
            <h3>Visualizzazione Dati Storici </h3>
            <button @click="chiudiModaleGrafico" class="btn-close">&times;</button>
          </div>
          <div class="p-4" style="padding-bottom: 0.5rem;">
            <PodChartCustom v-if="modaleGraficoAperto" :podId="podIdSelezionato" />
          </div>
        </div>
      </div>
                ...
                <div class="flex-gap">
                  ...
                  <div style="position: relative; flex: 1;">
                    <input 
                      ...
                      placeholder="Chiave segreta API" 
                      class="inp" 
                      style="width: 100%; padding-right: 40px;"
                    >
                    <button type="button" @click="showApiKey = !showApiKey" class="btn-eye">
                      <svg v-if="!showApiKey" viewBox="0 0 24 24" width="18" height="18" stroke="#94a3b8" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path>
                        <circle cx="12" cy="12" r="3"></circle>
                      </svg>
                      <svg v-else viewBox="0 0 24 24" width="18" height="18" stroke="#60a5fa" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round">
...
import MonitoringDashboard from '@/components/admin/MonitoringDashboard.vue';
...
// === VARIABILI PER LA MODALE DEL GRAFICO ===
const modaleGraficoAperto = ref(false);
const podIdSelezionato = ref(null);
...
// === GESTIONE MODALE GRAFICO ===
const apriModaleGrafico = (podId) => {
  console.log("🟢 [DEBUG] Bottone Grafico cliccato! POD ID selezionato:", podId);
  podIdSelezionato.value = podId;
  modaleGraficoAperto.value = true;
};

const chiudiModaleGrafico = () => {
  modaleGraficoAperto.value = false;
  podIdSelezionato.value = null; // Pulisce l'ID alla chiusura
};

// === GESTIONE MODALE FORM (Invariata) ===
...
.btn-back { display: inline-flex; align-items: center; gap: 8px; color: #94a3b8; text-decoration: none; font-size: 0.9rem; font-weight: 600; transition: color 0.2s; }
.btn-back:hover { color: #f8fafc; }
.page-header-compact { margin-bottom: 2rem; display: flex; justify-content: space-between; align-items: flex-end; }
...
.header-left p { color: #94a3b8; font-size: 0.85rem; margin-top: 4px; margin-bottom: 0;}
...
.slider { width: 38px; height: 22px; background-color: #475569; border-radius: 20px; position: relative; transition: 0.3s; margin-right: 10px; }
.slider:before { content: ""; position: absolute; height: 16px; width: 16px; left: 3px; bottom: 3px; background-color: white; border-radius: 50%; transition: 0.3s; }
...
.btn-text { background: none; border: none; cursor: pointer; font-weight: 600; font-size: 0.9rem; padding: 0; transition: 0.2s; display: inline-block; }
...
.modal-content { width: 100%; max-width: 650px; padding: 0; overflow: hidden; max-height: 90vh; overflow-y: auto; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.5); }
.modal-lg { max-width: 900px; } /* Più larga per ospitare bene il grafico */
