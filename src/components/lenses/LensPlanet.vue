<template>
  <div class="cards-stack fade-in">
    <div class="premium-card planet-hero-card">
      <p class="insight-subtitle">La tua impronta di carbonio annua:</p>
      <div class="co2-visual">
        <h2 class="co2-number">{{ displayCo2 }}</h2>
        <div class="co2-unit-box">
          <span class="co2-unit">Tonnellate di CO2</span>
          <span class="co2-sub">emesse dalla tua casa</span>
        </div>
      </div>
      <div class="suv-comparison">
        <span class="suv-icon">🚗</span>
        <p>È come se guidassi un SUV per <strong class="text-planet">{{ suvKm }} km</strong> ogni anno.</p>
      </div>
    </div>
    ...
    <div class="premium-card smart-card-planet hover-scale">
      <div class="smart-header">
        ...
        <span class="smart-label">LA TUA OPPORTUNITÀ ESG</span>
      </div>
      <h4 class="smart-solution-title">Entra nella CER locale 'Bologna Nord'</h4>
      <p class="smart-solution-desc">Neutralizza subito il <strong>30% delle tue emissioni</strong> producendo e condividendo energia pulita con la comunità.</p>
    </div>
  </div>
</template>
...
const suvKm = computed(() => (props.targetCo2 * 3500).toLocaleString('it-IT'));
...
// Parte l'animazione in automatico appena il componente viene montato nel DOM
onMounted(() => {
  animateCo2();
});
</script>
...
.co2-unit { font-size: 18px; font-weight: 800; color: var(--accent); letter-spacing: 1px; }
...
.text-planet { color: var(--accent); font-weight: 800; }
.smart-card-planet { background: linear-gradient(165deg, var(--bg-card) 50%, rgba(16, 185, 129, 0.12) 100%); border: 1px solid var(--accent); }

.smart-header { display: flex; align-items: center; gap: 8px; margin-bottom: 8px; }
.icon-box-small { color: var(--accent); width: 18px; height: 18px; font-size: 16px; }
.smart-label { font-size: 10px; font-weight: 900; color: var(--accent); letter-spacing: 1px; }
